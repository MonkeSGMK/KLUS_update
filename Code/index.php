<!--for future devs: succes >:)
    btw anil is degene die niet weet hoe je gereedschap spelt, ik kan het wel maar ik laat het zo want lazy -Jay -->

<!-- 
    Geachte toekomstige developers,
    gg ez no re. 
...
<body>
    ...
    <div class="top-box">
        ...
        <!-- it is done, we will now proceed to not remove these notes -->
        <div class="sort">
    ...
    <div class="container-fluid mt-4">
        <div class="row" id='itemsToSort'>
            ...
            if (isset($_POST['search'])) {
                ...
                $stmt = $conn->prepare('SELECT id, naam, aantal, image_path FROM Materiaal WHERE naam LIKE ?');
                $stmt->execute(['%' . $searchTerm . '%']);
                $result = $stmt->fetchAll();
            } else {
                $stmt = $conn->prepare('SELECT id, naam, aantal, image_path FROM Materiaal');
                $stmt->execute();
                $result = $stmt->fetchAll();
            }
            ...
            foreach ($result as $data) {
                ...
                echo '<a href="editAantal.php?type=materiaal&id=' . $data['id'] . '&amount=3">';
                ...
                echo '</a>';
                echo '<a href="editAantal.php?type=materiaal&id=' . $data['id'] . '&amount=2">';
                ...
                echo '</a>';
                echo '<a href="editAantal.php?type=materiaal&id=' . $data['id'] . '&amount=1">';
